...
def update_product_description(product_id, improved_description):
...
def product_description_ai_seo():
    try:
        # Check for the custom authorization header for security
        ...
        #ract description and id from the incoming request
        description = request.json.get('event', {}).get('data', {}).get('new', {}).get('description')
        product_id = request.json.get('event', {}).get('data', {}).get('new', {}).get('id')
        ...
        # Update the product description in Hasura
        updated_product_id = update_product_description(product_id, improved_description)

        # Return a success response
        return jsonify({'message': 'Success', 'product_id': updated_product_id}), 200
